...
exports.getClueScore = exports.generateCombinations = void 0;
const generateCombinations = (good_cards) => {
    const slow_queue = [];
    const result = [];
    const seen = new Set();
...
const getClueScore = (clue, trial_output, good_cards, bad_cards, neutral_cards, assassin_cards, risk_factor) => {
    ...
    if (n !== clue.intended.length + 1)
        return Number.NEGATIVE_INFINITY;
    let risk = 0;
    for (let i = 0; i < n - 1; i++) {
        const word = trial_output[i];
        const rsf = Math.pow((n - i - 1) / (n - 1), 2);
        if (good_set.has(word))
            risk += 0.5 + 0.5 * rsf; // 1.0  --> 0.5
        else if (neutral_set.has(word))
            risk += 4.0 + 6.0 * rsf; // 8.0  --> 4.0
        else if (bad_set.has(word))
            risk += 8.0 + 8.0 * rsf; // 12.0  --> 8.0
        else if (assassin_set.has(word))
            risk += 30.0 + 10.0 * rsf; // 40.0 --> 30.0
    }
    if (intended_set.has(trial_output[n - 1]))
        risk -= 0.5;
    return n - (1 - risk_factor) * risk - 1;
};
exports.getClueScore = getClueScore;
